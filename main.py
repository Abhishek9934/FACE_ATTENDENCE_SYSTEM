...
def index():
    ...
    for table in table_list:
        if (table=='StudentRecord'  or table=='TeacherRecord' or table=='new_table'):
            continue
...
def detect():
    ...
    if request.method == 'POST':
        ...
        os.system(f'python detect.py {arg1}')
...
def teacherlogin():
    if request.method == 'POST':
        ...
        mydb= CONNECTION()
        cursor= mydb.cursor()
        cursor.execute("SELECT Email FROM TeacherRecord  WHERE TeacherId=%s",(Id,))
        d= cursor.fetchone()
        print('reached here')
        # print(birth)
        if (d!=None and d[0] == str(email)):
